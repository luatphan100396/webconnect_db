...
P1: BEGIN
    ...
    (
	    ...
	    from GROUP_RESTRICTED_FIELD_TABLE gr
	...
	BEGIN
		...
		SELECT DISTINCT 
		        ...
		        fc.COMPONENT_KEY, 
		        fc.COMPONENT_NAME,
		        case when nrf.GROUP_KEY is null then 'All Field(s)' 
	...
	END;
	 ...
	 BEGIN
		...
		INNER JOIN FIELD_TABLE field 
		     ...
		     AND rf.GROUP_KEY = @v_GROUP_KEY 
		LEFT JOIN GROUP_RESTRICTED_FIELD_TABLE grf
		     ON grf.FIELD_KEY = field.FIELD_KEY
		     AND grf.GROUP_KEY = rf.GROUP_KEY
