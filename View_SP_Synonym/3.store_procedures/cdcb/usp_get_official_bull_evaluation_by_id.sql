...
(
 ...
 BEGIN
	...
	DECLARE sql_country_contribution_table_serialzied 		    VARCHAR(30000);
	...
	DECLARE GLOBAL TEMPORARY TABLE SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION -- drop table SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION 
	(
		ROOT_ANIMAL_ID CHAR(17),
		COUNTRY char(3),
		YIELD_HERDS int,
	...
	(
		...
		PED_GEN varchar(10),
		INBRD varchar(20),
		EXP_FUT_INBRD varchar(20),
		DAU_INBRD varchar(20)
	)with replace ON COMMIT PRESERVE ROWS;
	  ...
	  (
	    ...
	    CNTRL_STUD varchar(128),
	    ORIG_STUD varchar(128),
	    ...
	    INTERNATIONAL_ID_IND varchar(128),
	    LONG_NAME varchar(128),
	    SHORT_NAME varchar(128),
	    REG varchar(128),
	    RECESSIVES varchar(128),
	    SRC varchar(128),
	    HERD_WITH_MOST_DAU varchar(128),
	    MOST_DAU_HERD_QTY varchar(128),
	    ROW_ID INT GENERATED BY DEFAULT AS IDENTITY  (START WITH 1 INCREMENT BY 1) 
	  ) with replace ON COMMIT PRESERVE ROWS; 
   ...
   DECLARE GLOBAL TEMPORARY TABLE SESSION.TMP_RESULT_BV_MERIT
	  ...
	  (
	   ...
	   DECLARE GLOBAL TEMPORARY TABLE SESSION.TMP_RESULT_SCR
	  (
	    ROOT_ANIMAL_ID char(17),
	    SCR_PTA varchar(128),
	    SCR_REL varchar(128),
	    SCR_BREEDINGS varchar(128)  
	  ) with replace ON COMMIT PRESERVE ROWS;
	  ...
	  (
	   ...
	   DECLARE GLOBAL TEMPORARY TABLE SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION_SERIALIZED
	  (
	    ROOT_ANIMAL_ID char(17),
        VALUE varchar(30000) 
	  ) with replace ON COMMIT PRESERVE ROWS; 
   ...
   END IF;
	...
	select PUBRUN_PDATE, PUBRUN
	into v_EVAL_PDATE,v_PUBRUN
	FROM ENV_VAR_TABLE LIMIT 1;  
	...
	with UR; 
        ...
        with UR;
            ...
            -- Get bull dtrs
            INSERT INTO SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION 
            (
                    ROOT_ANIMAL_ID,
                    COUNTRY,
                    YIELD_HERDS,
            ...
            SELECT ROOT_ANIMAL_ID as ROOT_ANIMAL_ID,
		 	...
		 	( 
	            SELECT 
	                    bv.ROOT_ANIMAL_ID,
	                    'Pedigree' as PED_GEN, 
	                    bv.INBRD_PCT as INBRD,
	                    bv.FUTURE_DAU_INBRD_PCT as EXP_FUT_INBRD,
	                    bv.DAU_INBRD_PCT as DAU_INBRD
  ...
  INSERT INTO SESSION.TMP_RESULT_BV_EVALUATION		   
	SELECT ROOT_ANIMAL_ID AS ROOT_ANIMAL_ID,
			...
			float2char_thsnd_format(RELPA,1) || (case when RELPA is not null then '%' else '' end ) as RELPA
		 	...
		 	-- SCR
		 	INSERT INTO SESSION.TMP_RESULT_SCR
		 	SELECT  
		 	    animEvl.ROOT_ANIMAL_ID as ROOT_ANIMAL_ID
    ...
    IF @IS_DATA_EXCHANGE ='0' THEN    
  	-- Get animal information 
	    ...
	    begin
		 	...
		 	SELECT * FROM SESSION.TMP_RESULT_BV_MERIT;
    ...
    --DS7: Get animal BV PIA
	  ...
	  begin
		 	...
		 	SELECT * FROM SESSION.TMP_RESULT_BV_EVALUATION; 
		 	OPEN cursor4;
	   end;
 	   ...
 	   --DS9: Get animal BV dtrs
	     ...
	     begin
		 	...
		 	SELECT   *
		 	FROM SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION with UR;
	    ...
	    begin
		 	...
		 	SELECT * FROM SESSION.TMP_RESULT_SCR 
	        ;
      ...
      ELSEIF @IS_DATA_EXCHANGE ='1' THEN 
			...
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_MERIT_DATA') from sysibm.sysdummy1)||'''  
			                          
			                                            ) ) as VARCHAR(30000)),'||(length(prefix_empty_char)+1)||'),''&#xD;'','''')
			...
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_INBREEDING') from sysibm.sysdummy1)||'''  
			...
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_EVALUATION_DATA') from sysibm.sysdummy1)||'''  
			...
			set sql_country_contribution_table_serialzied= '
			insert into  SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION_SERIALIZED(ROOT_ANIMAL_ID,VALUE)
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_COUNTRY_CONTRIBUTION_DATA') from sysibm.sysdummy1)||'''  
			                          
			                                            ) ) as VARCHAR(30000)),'||(length(prefix_empty_char)+1)||'),''&#xD;'','''')
			from SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION
			group by ROOT_ANIMAL_ID
			...
			';
			  ...
			  execute immediate sql_country_contribution_table_serialzied ;
			...
			select  'SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION_SERIALIZED',
			       '',
			       'bv_country' 
			from sysibm.sysdummy1;
