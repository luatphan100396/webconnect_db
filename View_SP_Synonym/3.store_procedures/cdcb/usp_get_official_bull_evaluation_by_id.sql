 ...
 BEGIN
   ...
   END IF;
	...
	SELECT 
		...
		INNER JOIN SESSION.tmp_BULL_EVL_TABLE animEvl 
  ...
  (
    ...
    --DS7: Get animal BV PIA
	  ...
	  begin
 	   ...
 	   --DS9: Get animal BV dtrs
	     ...
	     begin
		 	...
		 	SELECT   *
		 	FROM SESSION.TMP_RESULT_COUNTRY_CONTRIBUTION  order by country;
      ...
      ELSEIF @IS_DATA_EXCHANGE ='1' THEN 
			...
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_DATA') from sysibm.sysdummy1)||'''  
			...
			select ROOT_ANIMAL_ID,  
			       replace(substr(xmlserialize(xmlagg(xmltext ( '''||prefix_empty_char||(select fn_DataExchange_Get_Serialized_String_by_template('BULL_OFFICAL_EVALUATION_COUNTRY_CONTRIBUTION_DATA') from sysibm.sysdummy1)||'''  
			                                            ...
			                                            )order by country ) as VARCHAR(30000)),'||(length(prefix_empty_char)+1)||'),''&#xD;'','''')
