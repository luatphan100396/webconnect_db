...
--Description: Add new/edit account
--Output:
...
P1: BEGIN
  ...
  --  SET retcode = SQLCODE; 
	...
	DECLARE GLOBAL TEMPORARY TABLE SESSION.TmpFilterinputsMultiSelect_Permission 
	(
		Field      VARCHAR(128),
		Value       VARCHAR(128),
		Order  smallint  GENERATED BY DEFAULT AS IDENTITY  (START WITH 1 INCREMENT BY 1)  
	...
	INSERT INTO SESSION.TmpFilterinputsMultiSelect_permission 
		(    
			Field,
			Value
		)
	...
	SET v_GROUP =  (   select value
     ...
     from SESSION.TmpFilterinputsMultiSelect_permission;		 
 	  ...
 	  -- correct id for matching with TmpAffiliate
 	  
 	  UPDATE SESSION.TmpAffiliatePermission SET AFFILIATE_ID = ID;
 	  
 	  MERGE INTO SESSION.TmpAffiliatePermission  as A
		 using
		 ( 
			 select t2.id,
			        t1.id as AFFILIATE_ID
			from
			SESSION.TmpAffiliatePermission t1
			left join SESSION.TmpAffiliatePermission t2
			on t1.source_short_name = t2.source_short_name
			and t1.id = t2.id-1
		 )AS B
		 ON  A.id = B.id 
		 WHEN MATCHED THEN UPDATE
		 SET A.AFFILIATE_ID = B.AFFILIATE_ID;
		...
		MERGE INTO SESSION.TmpAffiliatePermission  as A
		 using
		 ( 
			SELECT t.*, dense_rank() OVER ( order by AFFILIATE_ID) as new_AFFILIATE_ID
			from session.TmpAffiliatePermission t
		 )AS B
		 ON  A.id = B.id 
		 WHEN MATCHED THEN UPDATE
		 SET A.AFFILIATE_ID = B.new_AFFILIATE_ID;
	   ...
	   -- INPUT VALIDATION
		IF  v_FIRST_NAME IS NULL
			...
			OR v_USER_KEY IS NULL OR (@is_add_new='0' AND v_USER_KEY = 0)
		...
		END;
			 ...
			 ELSE 
				COMMIT ; 
				 ...
				 BEGIN
						DECLARE cursor12 CURSOR WITH RETURN for
						SELECT  1 AS RESULT 
						FROM sysibm.sysdummy1;
