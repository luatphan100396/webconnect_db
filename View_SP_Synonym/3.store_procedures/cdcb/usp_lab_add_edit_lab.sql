...
P1: BEGIN
  ...
  --  SET retcode = SQLCODE; 
	...
	SET v_DATA_SOURCE_KEY = (SELECT MAX(DATA_SOURCE_KEY) + 1 FROM DATA_SOURCE_TABLE with UR);
	...
	IF (EXISTS(SELECT 1 FROM DATA_SOURCE_TABLE WHERE LOWER(SOURCE_SHORT_NAME) = LOWER(v_SOURCE_SHORT_NAME)))
	THEN
 ...
 BEGIN  
     ...
     SET RETCODE = SQLCODE;
		  ...
		  MERGE INTO DATA_SOURCE_TABLE as dest
			using
			( 
				SELECT v_SOURCE_SHORT_NAME as SOURCE_SHORT_NAME
				FROM sysibm.sysdummy1 
			)AS src
			ON  dest.SOURCE_SHORT_NAME = src.SOURCE_SHORT_NAME 
