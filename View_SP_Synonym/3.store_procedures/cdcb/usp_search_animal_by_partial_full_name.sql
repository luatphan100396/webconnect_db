...
BEGIN
	...
	MERGE INTO SESSION.TmpInputs as A
	 ...
	 set sql_search_name = (select substr(xmlserialize(xmlagg(xmltext ( 'or anim_name like ''%'||upper(input_value)||'%'''
	                                             )  ) as VARCHAR(30000)),3)
	                        from  SESSION.TmpInputs                    
	                        );
	...
	-- DS1: output list
     	begin
		 	...
		 	SELECT   
			 	...
			 	a.SPECIES_CODE,
			 	a.SEX_CODE,
			 	t.INPUT_VALUE AS PARTIAL_FULL_NAME,
				...
				row_number()over(order by t.ORDER )  as ORDER
		 	FROM SESSION.TmpFullNameLists a
		 	inner join  SESSION.TmpInputs t 
		 	   on a.ANIM_NAME like '%'||upper(t.INPUT_VALUE)||'%'
