...
P1: BEGIN
   ...
   SET v_COUNT_NOMINATOR = (SELECT COUNT(1) FROM SESSION.TmpNominator);
	 ...
	 SELECT  
		...
		(
		    ...
		    inner join DATA_SOURCE_TABLE r 
		   		 on t.NOMINATOR = r.SOURCE_SHORT_NAME
		...
		WHERE 	
		(v_SEARCH_BY IS NULL 
			  OR ( uAccTable.USER_NAME LIKE '%'||v_SEARCH_BY||'%'
					OR uInfoTable.EMAIL_ADDR LIKE '%'||v_SEARCH_BY||'%'
					OR uInfoTable.FIRST_NAME LIKE '%'||v_SEARCH_BY||'%'
					OR uInfoTable.ORGANIZATION LIKE '%'||v_SEARCH_BY||'%'
				)
		)
		AND (v_GROUP is null or gr.GROUP_SHORT_NAME= v_GROUP)
	...
	LEFT JOIN DATA_SOURCE_TABLE r 
		 	ON ur.DATA_SOURCE_KEY = r.DATA_SOURCE_KEY
	WHERE SOURCE_SHORT_NAME='CA'
	OR SOURCE_SHORT_NAME='WI'
	OR SOURCE_SHORT_NAME='NC'
	OR SOURCE_SHORT_NAME='UT'
		group by t.USER_KEY
