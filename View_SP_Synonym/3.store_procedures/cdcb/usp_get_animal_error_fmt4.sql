CREATE OR REPLACE PROCEDURE usp_get_animal_error_fmt4
--==========================================================================================
--Author: Nghi Ta
...
BEGIN
  ...
  END WHILE;
 	...
 	SELECT
 	  SUBSTRING(BASE_RECORD,3,17) AS ANIMAL_ID 
	 	...
	 	,SUBSTRING(BASE_RECORD,79,1) AS SRC
	 	,trim(NULLIF(SUBSTRING(BASE_RECORD,80,8),v_empty_date)) AS PROC_DATE
	 	,SUBSTRING(BASE_RECORD,88,1) AS REC_TYPE
	 	,SUBSTRING(BASE_RECORD,89,1) AS VER 
	 	,SUBSTRING(BASE_RECORD,91,1) AS MBC
	 	,SUBSTRING(BASE_RECORD,100,1) AS ID_CHG
	 	,SUBSTRING(BASE_RECORD,101,1) AS PNT_CHG
	 	,SUBSTRING(BASE_RECORD,102,2) AS DPRC 
	 	,SUBSTRING(BASE_RECORD,104,3) AS AFFIL
	 	,CAST(HERD_CODE AS VARCHAR(10)) AS HERD_CODE
	 	,CAST(CTRL_NUM  AS VARCHAR(10)) AS CTRL_NUM 
	 	,SUBSTRING(BASE_RECORD,126,1) AS LAC_TYPE
	 	,SUBSTRING(BASE_RECORD,127,1) AS LAC_VER
		,VARCHAR_FORMAT(DEFAULT_DATE + CALV_PDATE,'YYYY-MM-DD') AS CALVING_DATE 
	 	,DIM_QTY AS DIM
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,139,3)),'') AS DRY
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,159,2)),'') AS LACT_NUM -- should be LACT_NUM
	 	,SUBSTRING(BASE_RECORD,161,1) AS TERMINATION_CODE
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,147,4)),'') AS MILK
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,151,4)),'') AS FAT
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,155,4)),'') AS PROTEIN
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,179,3)),'') AS SCS
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,144,3)),'') AS DAYS3X
	 	,NULLIF(trim(SUBSTRING(BASE_RECORD,176,2)),'') AS TOT
	 	,trim(NULLIF(SUBSTRING(BASE_RECORD,162,8),v_empty_date)) AS BREED_DT
	 	,SUBSTRING(BASE_RECORD,174,1) AS LACT_INIT_CODE
	 	,NULLIF(SUBSTRING(BASE_RECORD,243,1),'') AS NUM_PROG	 
	 	,ASCII(ERR_SEGMENT_QTY) AS ERR_SEGMENT_QTY
	 	,ERR_SEGMENT_RECORD
	 	,ASCII(TD_SEGMENT_QTY) AS TD_SEGMENT_QTY
	 	,TD_SEGMENT_RECORD 
	 	,SOURCE_FILE_NAME
