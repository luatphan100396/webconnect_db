CREATE OR REPLACE PROCEDURE usp_Get_Cow_Type_Evaluations
--========================================================================================================
--Author: Linh Pham
--Created Date: 2020-12-18
--Description: Get official cow evaluation data: type for one animal
--Output: 
...
P1: BEGIN
        ...
        DECLARE GLOBAL TEMPORARY TABLE SESSION.TmpAnimalLists_CV_type_PIA -- drop table SESSION.TmpAnimalLists_CV_type_PIA 
        (
                ROOT_INT_ID CHAR(17),
                TRAIT varchar(128), 
                PTA decimal(10,2),
...
--	   ; 
        ...
        SELECT  
                ...
                cv.REL16,
                cv.REL17		 
        FROM  COW_TYPE_TABLE cv
        INNER JOIN SESSION.TMP_INPUT inp
        ON cv.ANIM_KEY = inp.ANIM_KEY 
        AND cv.EVAL_PDATE = v_EVAL_PDATE
        AND cv.SPECIES_CODE = inp.SPECIES_CODE
	...
	UPDATE SESSION.TmpAnimalLists_CV_type_PIA  SET PTA = PTA/10.0;
