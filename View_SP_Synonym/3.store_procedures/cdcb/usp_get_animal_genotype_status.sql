CREATE OR REPLACE PROCEDURE usp_Get_Animal_Genotype_Status
--===================================================================================================
--Author: Linh Pham
...
--for one animal
--Output:
-- 		  +DS1: Genotype conflicts: conflict id, conflict bar code, confict position, error type, error code
   ...
   -- Populate data for tempory GENOTYPE_TABLE
	...
	(
		...
		SELECT    
			...
			gTable.SIRE_STATUS_CODE,
			gTable.DAM_STATUS_CODE,
			gTable.PROGENY_CNT,
			gTable.TISSUE_SOURCE_CODE,
			'' as PEND,
		...
		FROM SESSION.TmpAnimalLists_Geno_Sample gSample
...
-- Get genotype samples	
	BEGIN
			...
			SELECT ROOT_INT_ID AS ROOT_ANIMAL_ID,
					...
					DAM_STATUS_CODE,
					PROGENY_CNT,
					TISSUE_SOURCE_CODE AS TISSUE_SOURCE,
					CDCB_FEE_PAID_CODE AS FEE_CODE,
					PEND,
					LAB,
					REQUESTOR,
					GNO_ARRIVAL_DATE,
