...
BEGIN
   ...
   DECLARE sql_trait_code_1 varchar(30000);
   DECLARE sql_trait_code_gen varchar(30000);
   DECLARE sql_trait_code_trad varchar(30000);
   DECLARE sql_trait_code_std_dev varchar(30000); 
   DECLARE sql_trait_code_gen_rel varchar(30000);
   DECLARE sql_trait_code_trad_rel varchar(30000);
   ...
   SET v_IS_SHOW_TYPE_TRAIT = (SELECT case when count(1) >0 then 1 else 0 end FROM SESSION.v_TmpTraitList where type ='TYPE_TRAIT');
   
   SET BIRTH_PDATE_YOUNG_THRESHOLD = (select days('2004-01-01')  - days(DEFAULT_DATE)  from sysibm.sysdummy1);
    
   
   SET v_SUBSET_CODE_CONDITION = CASE WHEN v_SUBSET_CODE ='A_F' THEN 'bv.status_code in (''A'',''F'')'
   									  WHEN v_SUBSET_CODE ='Proven Bulls' THEN 'bv.PTA_MLK_QTY is not null'
   									  WHEN v_SUBSET_CODE ='Young Bulls' THEN 'bv.BIRTH_PDATE >= '||BIRTH_PDATE_YOUNG_THRESHOLD
   									  WHEN v_SUBSET_CODE ='Young Available Bulls' THEN 'bv.status_code =''G'' and bv.BIRTH_PDATE >= '||BIRTH_PDATE_YOUNG_THRESHOLD
   									  WHEN v_SUBSET_CODE ='Heifer' THEN 'bv.PTA_MLK_QTY is null'
                                      ELSE NULL
                                  END;
   ...
   select  
		substr(xmlserialize(xmlagg(xmltext(',float2char(avg(case when bv.GENOMICS_IND =1 then '||PTA_COLUMN_NAME||' else null end),'||DECIMAL_ADJUST||') as MEAN_GEN_PTA_'||t.trait||'
						 ,float2char(avg('||PTA_COLUMN_NAME||'),'||DECIMAL_ADJUST||') as MEAN_TRAD_PTA_'||t.trait||'
						 ,float2char(stddev('||PTA_COLUMN_NAME||'),'||DECIMAL_ADJUST||') as STD_DEV_PTA_'||t.trait 
						 ||case when t.trait not in ('FAT_PCT','PRO_PCT') then '
						 ,float2char(avg(case when bv.GENOMICS_IND =1 then '||REL_PTA_COLUMN_NAME||' else null end),'||DECIMAL_ADJUST||') as MEAN_GEN_REL_'||t.trait 
						         else ''
						    end 
						   || 
						   case when t.trait not in ('FAT_PCT','PRO_PCT') then '
						  ,float2char(avg('||REL_PTA_COLUMN_NAME||'),'||DECIMAL_ADJUST||') as MEAN_TRAD_REL_'||t.trait 
						         else ''
						    end 
						    ||'
						    ' 
   ...
   (
    select TRAIT,
           DECIMAL_ADJUST, 
           case when t.Type in ( 'TRAIT','INDEX')  then 'bv.PTA_'||t.trait||'_QTY' 
                when  t.Type in ('TYPE_TRAIT')  then 'typeTable.PTA'||t.trait||'*0.1'
                else ''
           end as PTA_COLUMN_NAME,
           case when t.Type in ( 'TRAIT','INDEX')  then 'bv.PTA_'||t.trait||'_REL_PCT' 
                when  t.Type in ('TYPE_TRAIT')  then 'typeTable.REL'||t.trait
                else ''
           end as REL_PTA_COLUMN_NAME,
           ORDER
  ...
  SET sql_query_main_from =  '(
	 ...
	 where eval_pdate =  '||case when v_SUBSET_CODE ='A_F' then v_PRE_EVAL_PDATE
	                             else v_EVAL_PDATE
	                        end
	                       || ' 
	  and eval_breed_code ='''||v_BREED_CODE||'''
	  and '||v_SUBSET_CODE_CONDITION||'
	 )ai_bull
...
--       select sql_query, v_IS_SHOW_TYPE_TRAIT
